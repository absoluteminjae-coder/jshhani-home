...
(function(){
  ...
  function applyDetailSeo(slug,pageName){
    const isMenopausePage=(slug==='menopause'||String(pageName||'').includes('갱년기'));
    const menopauseSeo=DETAIL_SEO.menopause||{};
    const seo=isMenopausePage?menopauseSeo:(DETAIL_SEO[slug]||{});
    ...
    const hasStaticSchema=[...document.head.querySelectorAll('script[type="application/ld+json"]')]
      .some(script=>script.id!=='jsh-detail-schema' && script.textContent.includes(canonical+'#webpage'));
    ...
    const graph=[
      {
        ...
        'inLanguage':'ko-KR',
        'dateModified':(slug==='damjeok'||slug==='bopye'||slug==='pain'||isMenopausePage)?'2026-09-17':undefined,
        'author':(slug==='damjeok'||slug==='bopye'||slug==='pain'||isMenopausePage)?{'@type':'Person','name':'차민재','jobTitle':'한의사','url':'https://jshhani.com/doctor.html','worksFor':{'@id':'https://jshhani.com/#clinic'}}:undefined,
    ...
    if(slug==='pain'){
      graph.push({
        ...
        '@id':canonical+'#faq',
        'mainEntity':[
          {'@type':'Question','name':'골반이 틀어져서 허리가 아픈 건가요?','acceptedAnswer':{'@type':'Answer','text':'모든 허리 통증의 원인이 골반인 것은 아닙니다. 허리, 관절, 근육, 인대, 신경 등의 상태를 확인하면서 골반과 고관절의 움직임이 현재 통증과 실제로 연관되는지를 함께 평가합니다.'}},
          {'@type':'Question','name':'골반 교정만 하면 통증이 없어지나요?','acceptedAnswer':{'@type':'Answer','text':'그렇지 않습니다. 움직임 문제가 중요한 환자도 있지만 조직 손상, 염증, 신경 자극, 유착이나 근육 약화가 함께 있다면 다른 치료가 필요할 수 있습니다.'}},
          {'@type':'Question','name':'초음파는 왜 사용하나요?','acceptedAnswer':{'@type':'Answer','text':'근육, 힘줄, 인대 등 연부조직을 확인하거나 초음파약침술을 시행할 때 목표 부위와 주변 구조를 구분하는 데 활용합니다.'}},
          {'@type':'Question','name':'매선은 어떤 환자에게 권하나요?','acceptedAnswer':{'@type':'Answer','text':'제세현한의원에서는 특히 근육 약화와 지지력 저하가 명확하고 통증이 반복되는 환자에게 매선치료를 적극적으로 고려합니다. 모든 통증 환자에게 동일하게 적용하지는 않습니다.'}},
          {'@type':'Question','name':'수술을 권유받았는데 한의원 진료도 가능한가요?','acceptedAnswer':{'@type':'Answer','text':'진료는 가능하지만 반드시 수술을 피해야 한다는 의미는 아닙니다. 진행성 근력저하나 중대한 신경 손상이 의심되는 경우에는 정형외과·신경외과 등의 평가가 우선될 수 있습니다.'}}
        ]
      });
    }
  ...
  async function renderDetail(){
    ...
    try{
      ...
      const isPainMainTemplate=(html)=>String(html||'').includes('data-pain-main-template="v31536"');
      const isMenopauseMainTemplate=(html)=>String(html||'').includes('data-menopause-main-template="v31541"');
      const isMenopausePage=(slug==='menopause'||String(p?.name||'').includes('갱년기'));
      ...
      const damjeokTemplate=(slug==='damjeok' && window.JSH_DAMJEOK_MAIN_TEMPLATE)
        ...
        : '';
      const painTemplate=(slug==='pain' && window.JSH_PAIN_MAIN_TEMPLATE)
        ? window.JSH_PAIN_MAIN_TEMPLATE
        : '';
      const menopauseTemplate=(isMenopausePage && window.JSH_MENOPAUSE_MAIN_TEMPLATE)
        ? window.JSH_MENOPAUSE_MAIN_TEMPLATE
        : '';
      ...
      if(slug==='pain' && painTemplate && !isPainMainTemplate(pinned?.content_html)){
        pinnedBody=painTemplate;
        pinnedTitle='통증클리닉';
      }

      if(isMenopausePage && menopauseTemplate && !isMenopauseMainTemplate(pinned?.content_html)){
        pinnedBody=menopauseTemplate;
        pinnedTitle='갱년기장애';
      }
      ...
      root.innerHTML=`
        ...
        <section class="featured">
          <div class="container">
            ...
            ${(pinned || (slug==='diet' && dietTemplate) || (slug==='bopye' && bopyeTemplate) || (slug==='damjeok' && damjeokTemplate) || (slug==='pain' && painTemplate) || (isMenopausePage && menopauseTemplate)) ? `
